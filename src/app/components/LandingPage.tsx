...
/* ─── ANIMATED CHAT LOOP ──────────────────────────────────────── */
// Fixed height layout: bubble → chat opens (fixed height always) → CTA shrinks msg area → loop restarts
const CHAT_HEIGHT = 380; // total fixed widget height in px
const CHAT_MSG_HEIGHT_DEFAULT = 200; // messages area height
const CHAT_MSG_HEIGHT_WITH_CTA = 148; // messages area shrinks when CTA appears
...
function AnimatedChatLoop({ theme }) {
  ...
  const runLoop = useCallback(() => {
    ...
    // Show bubble briefly, then open chat
    ...
    // Restart after CTA shown for 2.5s
  ...
  return (
    <div style={{ width: 300, position: 'relative' }}>
      ...
      {/* Chat widget — fixed total height */}
      <AnimatePresence>
        {chatOpen && (
          <motion.div key="chat"
          ...
          >
            <div style={{
              ...
              {/* Messages — height animates smoothly when CTA appears */}
              <motion.div
                ref={scrollRef}
              ...
              {/* CTA — slides in, pushes nothing, msg area shrinks */}
              <AnimatePresence>
                {showCTA && (
                  <motion.div
      ...
      {isDark && (
        <div className="absolute inset-0">
          <ImageWithFallback
            ...
            alt="AI Robot" className="w-full h-full object-cover" />
