...
function HeroAISlide({ onGetStarted }) {
  ...
  return (
    <section className="px-6 relative" style={{ display: 'flex', flexDirection: 'column', background: '#09090b', minHeight: '100vh', paddingTop: '56px' }}>
      ...
      <motion.div
        ...
        style={{ right: '18%', bottom: '4%', zIndex: 2 }}
      >
        {stats.map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ delay: i * 0.08 }}
            ...
            <div className="text-xs text-zinc-500 whitespace-nowrap">{s.label}</div>
          </motion.div>
        ))}
      </motion.div>
      <div className="max-w-6xl mx-auto w-full flex flex-col pt-4 pb-24 relative z-10">
  ...
  return (
    <div style={{ perspective: '1400px', overflow: 'hidden' }}>
      <motion.section ref={ref} style={{ y, scale, rotateX, opacity }}
        ...
        {/* Background image — mirrors hero layout */}
        <img
          src="/bg-nd.avif"
          alt=""
          className="absolute pointer-events-none select-none"
        ...
        <div className="relative z-10 flex items-center" style={{ minHeight: '100vh' }}>
          <div className="max-w-6xl mx-auto w-full px-6">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false, amount: 0.3 }}>
              ...
              <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ delay: 0.2 }}
              ...
              <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false }} transition={{ delay: 0.35 }}
...
/* ─── CTA FIRST (same as CTASlide but no scroll animation, used as landing page first slide) ── */
function CTASlideFirst({ onGetStarted }) {
  return (
...
export function LandingPage() {
  ...
  return (
    ...
    <div className="overflow-x-hidden bg-zinc-950" style={{ minHeight: '100vh', backgroundColor: '#09090b' }}>
      ...
      <CTASlideFirst onGetStarted={() => openLead()} />
