...
export function ChatWidgetLight() {
  return (
    <div
      style={{
        background: theme.bg,
    ...
    >
      ...
      {/* Messages */}
      <div className="flex flex-col gap-3 p-4" style={{ background: '#ececee' }}>
        {messages.map((msg, i) => (
          <motion.div
            key={i}
      ...
      {/* Chips */}
      <div className="flex gap-1.5 flex-wrap px-4 pb-2" style={{ background: '#ececee' }}>
        {['AI deployment', 'Pricing', 'Book a demo'].map(chip => (
          <span key={chip}
            style={{ color: theme.chipColor, border: `1px solid ${theme.border}`, background: 'rgba(255,255,255,0.7)' }}
